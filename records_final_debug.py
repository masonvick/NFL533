...
# Debug: Check teams in postseason data
print("Postseason teams:")
print(postseason[["season", "home_team", "away_team"]].drop_duplicates())

# Identify teams that played in postseason
postseason_teams = set(postseason["home_team"]).union(set(postseason["away_team"]))

# Debug: Compare postseason teams with made_playoffs
made_playoffs_teams = set(regular_season_standings["team"][regular_season_standings["team"].isin(postseason_teams)])
print("Teams in postseason games:", postseason_teams)
print("Teams marked as made_playoffs:", made_playoffs_teams)
print("Teams marked but not in postseason games:", made_playoffs_teams - postseason_teams)

# Add playoff qualification to regular season standings
regular_season_standings["made_playoffs"] = regular_season_standings["team"].isin(postseason_teams).astype(int)
...
# Debug: Check teams with discrepancies
discrepancies = final_standings[
    (final_standings["made_playoffs"] == 1) &
    (final_standings["playoff_wins"] == 0) &
    (final_standings["playoff_losses"] == 0)
]
print("Discrepancies (made_playoffs == 1 but no postseason record):")
print(discrepancies)
